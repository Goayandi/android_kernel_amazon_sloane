...
static struct kobject *add_ep_kobj;
...
#define OVER_CURRENT_DEVICE 105
...
static int scheduler_add_ep(struct xhci_hcd *xhci, struct sch_ep *sep)
{
	...
	int ret = SCH_SUCCESS;
	char *overCurrent_event[2] = { "USB_STATE=OVER_CURRENT", NULL };
	char *normalCurent_event[2] = { "USB_STATE=NORMAL_CURRENT", NULL };
	...
	if (gpio_get_value(OVER_CURRENT_DEVICE)) {
		xhci_err(xhci, "Over current checked normal --\n");
		uevent_envp = normalCurent_event;
		kobject_uevent_env(add_ep_kobj, KOBJ_CHANGE, uevent_envp);
	} else {
		xhci_err(xhci, "!!! over current checked Abnormal ---\n");
		uevent_envp = overCurrent_event;
		kobject_uevent_env(add_ep_kobj, KOBJ_CHANGE, uevent_envp);
	}
...
int xhci_mtk_init_quirk(struct xhci_hcd *xhci)
{
	...
	add_ep_kobj = kobject_create_and_add("add_ep", NULL);
	if (!add_ep_kobj)
		return -ENOMEM;

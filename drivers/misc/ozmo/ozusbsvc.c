 ...
 * Context: softirq
 */
int oz_usb_get_bus_addr(struct oz_pd *pd)
{
	struct oz_usb_ctx *usb_ctx;
	int bus_addr = 0;
	spin_lock_bh(&pd->app_lock[OZ_APPID_USB-1]);
	...
	if (usb_ctx == NULL)
		return -1;
	if (usb_ctx->hport)
		bus_addr = oz_hcd_get_bus_addr(usb_ctx->hport);
	oz_usb_put(usb_ctx);
	return bus_addr;
}
/*------------------------------------------------------------------------------
...
int oz_usb_stream_create(void *hpd, u8 ep_num)
{
	...
	oz_trace_msg(M, "oz_usb_stream_create(%02X)\n", ep_num);

	if (ep_num & 0x80) {
		ep_num &= 0x7F;
		usb_ctx->ref_stream |= 1 << ep_num;
	} else {
		usb_ctx->stream |= 1 << ep_num;
	}
...
int oz_usb_stream_delete(void *hpd, u8 ep_num)
{
	...
	oz_trace_msg(M, "oz_usb_stream_delete(%02X)\n", ep_num);
...
{
	return oz_usb_stream_create(hpd, ep_num|0x80);
}
/*------------------------------------------------------------------------------
 * Context: softirq
 */
...
{
	return oz_usb_stream_delete(hpd, ep_num|0x80);
}
/*------------------------------------------------------------------------------
 * Context: softirq
 */
int oz_usb_ref_clock_send(void *hpd, u8 ep_num)
{
	struct oz_usb_ctx *usb_ctx = (struct oz_usb_ctx *)hpd;
	struct oz_pd *pd = usb_ctx->pd;
	u8 ep_bit = 1 << (ep_num & 0x7F);
	int unit = OZ_QUANTUM;

	if ((!(usb_ctx->stream & ep_bit)) && (usb_ctx->ref_stream & ep_bit)) {
		oz_trace_msg(M, "Ref CLK\n");

		while (unit != 0) {
			oz_send_isoc_unit(pd, ep_num & 0x7F, NULL, 0);
			unit--;
		}
	}
	return 1;

...
struct oz_pd *oz_pd_alloc(const u8 *mac_addr)
{
	...
	if (pd) {
		...
		/* init battery level to 100 by default */
		pd->battery_level = 100;
		pd->reset_retry = 0;
...
void oz_pd_destroy(struct oz_pd *pd)
{
	...
	if (pd->pd_destroy_scheduled) {
		oz_trace_msg(M, "%s: not rescheduling oz_pd_free\n", __func__);
	...
	if (!ret)
		oz_trace_msg(M, "failed to schedule workitem\n");
	spin_unlock_bh(&pd->pd_destroy_lock);
}
/*------------------------------------------------------------------------------
 */
static void oz_pd_uevent_workitem(struct work_struct *work)
{
	...
	envp[0] = mac_buf;
	envp[1] = NULL;
	kobject_uevent_env(&g_oz_wpan_dev->kobj, pd->action, envp);
	pd->action = KOBJ_CHANGE;
	oz_pd_put(pd);
}
/*------------------------------------------------------------------------------
 */
void oz_pd_notify_uevent(struct oz_pd *pd)
{
	...
	if (!ret) {
		oz_trace_msg(M, "%s: failed to schedule workitem\n", __func__);
		oz_pd_put(pd);
	}
}
/*------------------------------------------------------------------------------
...
void oz_pd_stop(struct oz_pd *pd)
{
	...
	oz_polling_lock_bh();
	oz_pd_indicate_farewells(pd, OZ_FAREWELL_ON_KEEP_ALIVE);
...
int oz_pd_sleep(struct oz_pd *pd)
{
	...
	if (pd->keep_alive && pd->session_id) {
		if (pd->keep_alive >= OZ_KALIVE_INFINITE) {
			oz_pd_indicate_farewells(pd, OZ_FAREWELL_ON_SLEEP);
			oz_pd_indicate_farewells(pd, OZ_FAREWELL_ON_KEEP_ALIVE);
		} else {
			oz_pd_indicate_farewells(pd, OZ_FAREWELL_ON_SLEEP);
		}
...
static void oz_retire_frame(struct oz_pd *pd, struct oz_tx_frame *f)
{
	...
	if (f == NULL) {
		oz_trace_msg(M, "%s: oz_tx_frame is null\n", __func__);
		return;
	}
	e = f->elt_list.next;
	while (e != &f->elt_list) {
...
int oz_isoc_stream_create(struct oz_pd *pd, u8 ep_num)
{
	...
	st2 = pd_stream_find(pd, ep_num);
	if (st2 == NULL) {
		list_add(&st->link, &pd->stream_list);
		st = NULL;
	} else {
		/* stream exists. Increment ref-count */
		st2->ref_count++;
	}
...
void oz_pd_indicate_farewells(struct oz_pd *pd, u8 condition)
{
	...
	list_for_each_entry(f, &pd->farewell_list, link) {
		if (ai->farewell) {
			/* If condition is ON_KEEP_ALIVE, then all farewell
			 * messages are handled.
			 * If condition is ON_SLEEP, the flagged messag are
			 * handled.
			 */
			if (condition == (f->index & OZ_FAREWELL_CONDITION)) {

/* -----------------------------------------------------------------------------
Change-Id: Id96cbe9a35c2af6674eb2190783c2c7306174ee7
		...
		const u8 *pd_addr, struct net_device *net_dev, u32 pkt_num)
{
	...
	struct oz_pd *free_pd2 = NULL;
	oz_trace("%s()\n", __func__);
	oz_trace("Conn req: mode(%02x) pd_info(%02x) session_id(%02x) presleep(%02x)"
			" keep_alive(%02x) apps(%02x)\n",
			body->mode, body->pd_info, body->session_id,
			body->presleep, body->keep_alive, body->apps);
	if (cur_pd) {
		pd = cur_pd;
	...
	if (pd->mode & OZ_F_ISOC_NO_ELTS) {
		pd->ms_per_isoc = (body->ms_per_isoc>>2)*4;
		if (!pd->ms_per_isoc)
			pd->ms_per_isoc = 4;
			...
			const u8 *report, u8 len)
{
	...
	/* Get rid of condition field */
	index = index & OZ_FAREWELL_INDEX_MASK;
	list_for_each_entry(f2, &pd->farewell_list, link) {
		if ((f2->ep_num == ep_num) &&
		((f2->index & OZ_FAREWELL_INDEX_MASK) == index)) {
			found = 1;
			list_del(&f2->link);
			break;
		}
...
static void oz_rx_frame(struct sk_buff *skb)
{
	...
	if (pd) {
		...
		if ((current_time.tv_sec != pd->last_rx_timestamp.tv_sec) ||
			(pd->presleep <= 3 * MSEC_PER_SEC) || state_changed)  {
			oz_timer_add(pd, OZ_TIMER_TOUT,	pd->presleep);
...
enum hrtimer_restart oz_pd_heartbeat_event(struct hrtimer *timer)
{
	...
	/* Check if tasklet is already schedule.*/
	if (!test_and_set_bit(OZ_TASKLET_SCHED_HEARTBEAT, &pd->tasklet_sched)) {
		/* Check if tasklet is still in execution. */
		if (tasklet_trylock(&pd->heartbeat_tasklet)) {
			tasklet_unlock(&pd->heartbeat_tasklet);
			/* schedule tasklet! */
...
enum hrtimer_restart oz_pd_timeout_event(struct hrtimer *timer)
{
	...
	if (!test_and_set_bit(OZ_TASKLET_SCHED_TIMEOUT, &pd->tasklet_sched)) {
		/* Check if tasklet is still in execution. */
		if (tasklet_trylock(&pd->timeout_tasklet)) {
			tasklet_unlock(&pd->timeout_tasklet);
			/* schedule tasklet! */
...
void oz_binding_add(const char *net_dev)
{
	...
	if (binding) {
		...
		if (net_dev && *net_dev) {
			...
			oz_trace_msg(M, "dev_get_by_name(%s)\n", net_dev);
			binding->ptype.dev =
...
{
	struct oz_binding *binding, *n;
	int found = 0;

	oz_trace_msg(M, "Removing binding: '%s'\n", net_dev);
	spin_lock_bh(&g_binding_lock);
	list_for_each_entry_safe(binding, n, &g_binding, link) {

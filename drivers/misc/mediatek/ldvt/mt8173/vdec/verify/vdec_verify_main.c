...
void vVDecTouch(UINT32  u4InstID);
...
extern void vVerVP8DecEndProc(UINT32 u4InstID);
...
void vVDecVerifyThread(void* vpdata)
{
	...
	if(_u4CodecVer[u4InstID] == VDEC_VP8)
	{
        ...
        printk("_u4VerBitCount:%d, u4FileLength:%d\n", _u4VerBitCount[u4InstID], (_tInFileInfo[u4InstID].u4FileLength << 3));
        printk("vMpvPlay done\n");
...
void vVDecTouch(UINT32  u4InstID)
{
    ...
    printk("%s @ %d u4InstId %d \n", __FUNCTION__, __LINE__, u4InstID);

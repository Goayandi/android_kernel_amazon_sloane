...
#define CONFIG_CMDQ_DMAPOOL_SUPPORT 1

#if CONFIG_CMDQ_DMAPOOL_SUPPORT
...
static char *cmdq_dmapool_name[] = {
	...
	"cmdq_pool-4096",
	"cmdq_pool-8192",
	"cmdq_pool-16384" /* 16KB */
};
static struct dma_pool *g_cmdq_dma_pool[CMDQ_DMA_POOL_COUNT];
static bool b_cmdq_pool_created;
#endif
...
#if CONFIG_CMDQ_DMAPOOL_SUPPORT
void cmdq_core_dma_pool_deinitialize(void)
{
	int i;
	for (i = 0; i < CMDQ_DMA_POOL_COUNT; i++) {
		if (g_cmdq_dma_pool[i]) {
			dma_pool_destroy(g_cmdq_dma_pool[i]);
			g_cmdq_dma_pool[i] = NULL;
		}
	}
	b_cmdq_pool_created = false;
}

void cmdq_core_dma_pool_initialize(void)
{
	int i;
	if (!b_cmdq_pool_created) {
		for (i = 0; i < CMDQ_DMA_POOL_COUNT; i++) {
			g_cmdq_dma_pool[i] = dma_pool_create(cmdq_dmapool_name[i], cmdq_dev_get(),
								CMDQ_DMA_POOL_BUFFER_SIZE_BASE << i, 16, 0);
			if (!g_cmdq_dma_pool[i]) {
				CMDQ_ERR("Failed to create dma pool for cmdq\n");
				b_cmdq_pool_created = false;
				break;
			} else {
				b_cmdq_pool_created = true;
			}
		}
		...
		if (b_cmdq_pool_created)
			...
			cmdq_core_dma_pool_deinitialize();
	}

}

bool cmdq_core_find_dma_pool(size_t size, uint32_t *sizeIndex)
{
	int i;
	for (i = 0; i < CMDQ_DMA_POOL_COUNT; i++) {
...
void *cmdq_core_alloc_dma_pool(struct device *dev, size_t size, dma_addr_t *dma_handle,
				const gfp_t flag, uint32_t *usedIndex)
{
	void *pVA;
	dma_addr_t PA;
	int sizeIndex = 0xff;
	...
	do {
		...
		if (b_cmdq_pool_created) {
			cmdq_core_find_dma_pool(size, &sizeIndex);
			if (sizeIndex != 0xff) {
				pVA = dma_pool_alloc(g_cmdq_dma_pool[sizeIndex], flag, &PA);
				*usedIndex = 1<<sizeIndex;
			}
		}
...
void cmdq_core_free_dma_pool(struct device *dev, void *cpu_addr, dma_addr_t dma_handle,
									uint32_t usedIndex)
{
	if (b_cmdq_pool_created && (usedIndex < CMDQ_DMA_POOL_COUNT))
		dma_pool_free(g_cmdq_dma_pool[usedIndex], cpu_addr, dma_handle);
	else
		CMDQ_ERR("fail to free dma pool, b_cmdq_pool_created=%d usedIndex=%d\n",
					b_cmdq_pool_created, usedIndex);
}
#endif
...
#if CONFIG_CMDQ_DMAPOOL_SUPPORT
		} else if (pTask->dmaPoolUsed) {
			cmdq_core_free_dma_pool(cmdq_dev_get(), pTask->pVABase, pTask->MVABase,
									__builtin_ffs(pTask->dmaPoolUsed) - 1);
			pTask->dmaPoolUsed = 0;
#endif
		} else {
...
DEBUG_STATIC int32_t cmdq_core_task_realloc_buffer_size(struct TaskStruct *pTask, uint32_t size)
{
	...
	do {
#if CONFIG_CMDQ_DMAPOOL_SUPPORT
		pNewBuffer = cmdq_core_alloc_dma_pool(cmdq_dev_get(),
						       size, &newMVABase,
						       GFP_KERNEL, &dmaUsed);
...
DEBUG_STATIC void cmdq_core_release_task_unlocked(struct TaskStruct *pTask)
{
	...
	if (pTask->regResults) {
		CMDQ_MSG("COMMAND: Free result buf VA:0x%p, PA:%pa\n", pTask->regResults,
			 &pTask->regResultsMVA);
#if CONFIG_CMDQ_DMAPOOL_SUPPORT
		if ((pTask->regCount * sizeof(pTask->regResults[0])) <=
			(CMDQ_DMA_POOL_BUFFER_SIZE_BASE<<(CMDQ_DMA_POOL_COUNT-1))) {
			size_t bufferSize = pTask->regCount * sizeof(pTask->regResults[0]);
			uint32_t sizeIndex = 0xff;
			cmdq_core_find_dma_pool(bufferSize, &sizeIndex);
			cmdq_core_free_dma_pool(cmdq_dev_get(), pTask->regResults,
									pTask->regResultsMVA, sizeIndex);
		} else
#endif
		cmdq_core_free_hw_buffer(cmdq_dev_get(),
	...
	if (0 == pTask->regCount && extraBufferSize == 0) {
		...
		if (pTask->regCount) {
			...
			if ((pTask->regCount * sizeof(pTask->regResults[0])) <=
				(CMDQ_DMA_POOL_BUFFER_SIZE_BASE<<(CMDQ_DMA_POOL_COUNT-1))) {
				uint32_t tempIndex;
				pTask->regResults = cmdq_core_alloc_dma_pool(cmdq_dev_get(),
											pTask->regCount *
											sizeof(pTask->regResults[0]),
											&pTask->regResultsMVA,
											GFP_KERNEL, &tempIndex);
			} else
#endif
	...
	do {
		...
		*paStart = 0;
		...
		if ((count * sizeof(uint32_t)) <=
			(CMDQ_DMA_POOL_BUFFER_SIZE_BASE<<(CMDQ_DMA_POOL_COUNT-1))) {
			uint32_t tempIndex;
			pWriteAddr->va = cmdq_core_alloc_dma_pool(cmdq_dev_get(),
...
#if CONFIG_CMDQ_DMAPOOL_SUPPORT
			if ((sizeof(uint32_t) * pWriteAddr->count) <=
				(CMDQ_DMA_POOL_BUFFER_SIZE_BASE<<(CMDQ_DMA_POOL_COUNT-1))) {
				size_t bufferSize = sizeof(uint32_t) * pWriteAddr->count;
				uint32_t sizeIndex = 0xff;
				cmdq_core_find_dma_pool(bufferSize, &sizeIndex);
				cmdq_core_free_dma_pool(cmdq_dev_get(), pWriteAddr->va,
										pWriteAddr->pa, sizeIndex);
			} else
#endif
			cmdq_core_free_hw_buffer(cmdq_dev_get(),

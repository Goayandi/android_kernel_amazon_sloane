						...
						for (i = 1; (i + 2) <= blockLength; i += 3) {	/* Must loop in steps of 3 (SAD size) */
							...
							if ((blockPtr) <
								...
								if (((pDis->EdidBlock[blockPtr +
									...
									pDis->
									    EdidAFmts
									    [pDis->EdidSadCnt].ModeChans
									    =
									    pDis->EdidBlock[blockPtr
											    + i];
									pDis->
									    EdidAFmts
									    [pDis->EdidSadCnt].Freqs
									    =
									    pDis->EdidBlock[blockPtr
											    + i +
											    1];
					...
					case E_CEA_VSDB:	/* We have a VSDB */
						...
						if (blockLength >= 5) {
							if ((blockPtr) < (EDID_BLOCK_SIZE - 5)) {
								if ((pDis->EdidBlock[blockPtr + 1]
								     == 0x03)
								    &&
								    (pDis->EdidBlock[blockPtr + 2]
								     == 0x0C)
								    &&
								    (pDis->EdidBlock[blockPtr + 3]
								     == 0x00)) {
									pDis->EdidSinkType =
									    HDMITX_SINK_HDMI;
									if ((blockPtr) <
						...
						if (blockLength >= 8) {
							if ((blockPtr) < (EDID_BLOCK_SIZE - 10)) {
								...
								if ((pDis->EdidBlock[blockPtr +
									...
									if ((blockPtr) <
									    (EDID_BLOCK_SIZE -
									     12)) {
										pDis->
										    EdidLatency.Edidvideo_latency
										    =
										    pDis->EdidBlock
										    [blockPtr + 9];
										pDis->
										    EdidLatency.Edidaudio_latency
										    =
										    pDis->EdidBlock
										    [blockPtr + 10];
										pDis->
										    EdidLatency.EdidIvideo_latency
										    =
										    pDis->EdidBlock
										    [blockPtr + 11];
										pDis->
										    EdidLatency.EdidIaudio_latency
										    =
										    pDis->EdidBlock
										    [blockPtr + 12];

										pDis->
										    EdidLatency.latency_available
										    = True;
										pDis->
										    EdidLatency.Ilatency_available
										    = True;
								...
								pDis->
								    EdidExtraVsdbData.hdmiVideoPresent
								    =
								    (pDis->EdidBlock[blockPtr + 8] &
								     0x20) >> 5;

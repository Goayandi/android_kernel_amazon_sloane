...
int andes_wow_set_ssid_conf(struct _RTMP_ADAPTER *ad)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("%s, p2p_cfg.Ssid:%s, ssid len:%u, wake time:%x, extmode %x\n"
...
{				/* , RTMP_ADAPTER *ad, u32 cal_id, u32 param) */
	...
	if (ad->WOW_Cfg.extMode & WOW_FAKE_SUSPEND) {
		if (RTMP_CFG80211_VIF_P2P_GO_ON(ad)) {
			...
			pNetDev = RTMP_CFG80211_FindVifEntry_ByType(ad, RT_CMD_80211_IFTYPE_P2P_GO);
			if (pNetDev)
				RTMP_OS_NETDEV_STOP_QUEUE(pNetDev);
			else
				CFG80211DBG(RT_DEBUG_ERROR,
					("%s, P2P no netdev cannot stop queue", __func__));
		}
		IF_DEV_CONFIG_OPMODE_ON_STA(ad) {
			struct wifi_dev *wdev = &ad->StaCfg.wdev;
			PNET_DEV pNetDev = NULL;
			pNetDev = wdev->if_dev;
			if (pNetDev)
				RTMP_OS_NETDEV_STOP_QUEUE(pNetDev);
				...
				CFG80211DBG(RT_DEBUG_ERROR,
					("%s, STA no netdev cannot stop queue", __func__));
		}
	}
	...
	DBGPRINT(RT_DEBUG_ERROR, ("central channel = %d, WOW:%d, hw:%d\n",
...
error:
	...
	if (ad->WOW_Cfg.extMode & WOW_FAKE_SUSPEND) {
		if (RTMP_CFG80211_VIF_P2P_GO_ON(ad)) {
			...
			pNetDev = RTMP_CFG80211_FindVifEntry_ByType(ad, RT_CMD_80211_IFTYPE_P2P_GO);
			if (pNetDev)
				RTMP_OS_NETDEV_START_QUEUE(pNetDev);
			else
				DBGPRINT(RT_DEBUG_ERROR,
					("%s, P2P no netdev cannot resume queue", __func__));
		}
		IF_DEV_CONFIG_OPMODE_ON_STA(ad) {
			struct wifi_dev *wdev = &ad->StaCfg.wdev;
			PNET_DEV pNetDev = NULL;
			...
			if (pNetDev)
				...
				DBGPRINT(RT_DEBUG_ERROR,
					("%s, STA no netdev cannot resume queue", __func__));
		}
	}
	return ret;
}

...
static int XTAL_CLK_DEF = XTAL_24M;
...
int hdmi_driver_notifier_register(struct notifier_block *nb)
{
	...
	ret = blocking_notifier_chain_register(&blocking_notifier_list, nb);
	return ret;
}
...
int hdmi_driver_notifier_unregister(struct notifier_block *nb)
{
	...
	ret = blocking_notifier_chain_unregister(&blocking_notifier_list, nb);
	return ret;
}
...
int notify_power_status_change(unsigned long status)
{
	...
	ret = blocking_notifier_call_chain(&blocking_notifier_list, status, NULL);
	return ret;
}
...
unchar sp_tx_aux_dpcdwrite_bytes(unchar addrh, unchar addrm, unchar addrl, unchar cCount,
				 unchar *pBuf)
{
	unchar c, i;
	int ret = 0;
	...
	if (ON == 0) {
		...
		for (i = 0; i < 5; i++) {
			sp_write_reg_and(TX_P2, TX_PLL_FILTER5,
					 (~P5V_PROTECT_PD & ~SHORT_PROTECT_PD), &ret);
...
void hdmi_rx_initialization(void)
{
	...
	sp_write_reg_or(RX_P0, RX_SRST, HDCP_MAN_RST | SW_MAN_RST | TMDS_RST | VIDEO_RST, &ret);
...
#ifdef CONFIG_AMAZON_METRICS_LOG
	snprintf(buf, sizeof(buf), "slimport:def:edit_block=%d;CT;1:NR", (int)(c + 1));
...
{
	if ((pBuf[0] == 0) && (pBuf[7] == 0) && (pBuf[1] == 0xff) && (pBuf[2] == 0xff)
	    && (pBuf[3] == 0xff)
...
#ifdef CONFIG_AMAZON_METRICS_LOG
	snprintf(buf, sizeof(buf), "slimport:def:read_edit_times=%d;CT;1:NR", times);
	log_to_metrics(ANDROID_LOG_INFO, "kernel", buf);
	snprintf(buf, sizeof(buf), "slimport:def:block_sum=%d;CT;1:NR", (int)block_sum);
	log_to_metrics(ANDROID_LOG_INFO, "kernel", buf);
#endif
	return block_sum;
}
...
bool sp_tx_edid_read(unchar *pedid_blocks_buf)
{
	...
	if (bedid_print) {
		...
		for (k = 0; k < (128 * ((uint) blocks_num + 1)); k++) {
			if ((k & 0x0f) == 0)
				debug_printf("\n edid: [%.2x]  %.2x  ", (uint) (k % 0x0f),
					     (uint) pedid_blocks_buf[k]);
			else
...
void slimport_link_training(void)
{
	...
	case LT_INIT:
		...
		sp_tx_send_message((g_hdmi_dvi_status ==
				    HDMI_MODE) ? MSG_INPUT_HDMI : MSG_INPUT_DVI);
...
static void sp_tx_load_packet(enum PACKETS_TYPE type)
{
	...
	case AVI_PACKETS:
		...
		for (i = 0; i < 13; i++) {
			ret = sp_write_reg(TX_P2, SP_TX_AVI_DB0 + i, sp_tx_packet_avi.AVI_data[i]);
			if (unlikely(ret < 0))
				break;
		}
	...
	case SPD_PACKETS:
		...
		for (i = 0; i < 25; i++) {
			ret = sp_write_reg(TX_P2, SP_TX_SPD_DB0 + i, sp_tx_packet_spd.SPD_data[i]);
			if (unlikely(ret < 0))
				break;
		}
	...
	case VO_WAIT_VIDEO_STABLE:
		...
		if ((temp_value & (TMDS_DE_DET | TMDS_CLOCK_DET)) == 0x03) {
			...
			if (counter_unstable_video++ > 50) {	/* about 5 seconds  */
				debug_puts
				    ("Resterat due to too many HDMI input video not stable errors\n");
				sp_tx_set_sys_state(STATE_INIT);
				return;
			}
...
#ifdef CONFIG_AMAZON_METRICS_LOG
			log_to_metrics(ANDROID_LOG_INFO, "kernel", "slimport:def:DVI Mode;CT;1:NR");
#endif
...
{
	static unchar ds_vid_stb_cntr = 0, HDCP_fail_count;
...
static void sp_tx_enable_audio_output(unchar benable)
{
	...
	if (benable) {
		...
		if (c & AUD_EN_CTRL) {	/* if it has been enabled, disable first. */
			c &= ~AUD_EN_CTRL;
...
void slimport_cable_monitor(void)
{
	...
	if (sp_tx_system_state_bak != sp_tx_system_state
	    && HDCP_state != HDCP_WAITTING_FINISH && sp_tx_LT_state != LT_WAITTING_FINISH) {
...
void hdcp_external_ctrl_flag_monitor(void)
{
	...
	if ((sp_tx_rx_type == DWN_STRM_IS_ANALOG) || (sp_tx_rx_type == DWN_STRM_IS_VGA_9832)) {
...
static void sp_tx_hdcp_link_chk_fail_handler(void)
{
	if (slimport_is_cable_connected() == 0) {
		...
		system_state_change_with_case(STATE_HDCP_AUTH);
#ifdef CONFIG_AMAZON_METRICS_LOG
		log_to_metrics(ANDROID_LOG_INFO, "kernel", "slimport:def:HDCP Sync lost;CT;1:NR");
#endif
...
#ifndef HDCP_AUTO_EN
		if (HDCP_state > HDCP_WAITTING_FINISH || sp_tx_system_state > STATE_HDCP_AUTH)
#endif
		{
				...
				case 0x08:
					if ((Int_vector[0] & 0x10) != 0x10)
						...
						if (sp_tx_system_state >= STATE_HDCP_AUTH) {
							...
							debug_puts
							    ("Re-authentication due to downstream HDCP failure!\n");
							sp_tx_clean_hdcp_status(&ret);
						}
#else
						/* sp_tx_clean_hdcp_status(); */
...
static void sp_tx_auth_done_int_handler(void)
{
	...
	unchar bytebuf[2];
	int ret = 0;
	if (HDCP_state > HDCP_HW_ENABLE && sp_tx_system_state == STATE_HDCP_AUTH) {
...
{
	unchar c;
	if (sp_tx_LT_state == LT_WAITTING_FINISH && sp_tx_system_state == STATE_LINK_TRAINING) {
